...
interface TrafficInsightData {
  trafficPattern: string
  modelInfo?: ModelInfo | null
}

interface ReferrerInsightData {
...
function isSectionTitle(text: string): boolean {
  const titleKeywords = ["인사이트", "분석", "제안", "플랜", "전략", "추천", "시간대", "액션", "아이템", "요약", "현황", "트렌드", "세그먼트", "클러스터", "고객"]
...
export function AiInsights({ analytics, selectedUrl }: AiInsightsProps) {
  const [trafficInsights, setTrafficInsights] = useState<TrafficInsightData | null>(null)
  ...
  const generateTrafficInsights = async () => {
    ...
    try {
      ...
      setTrafficInsights({ trafficPattern: data.trafficPattern, modelInfo: data.modelInfo })
    } catch (err) {
      setErrorTraffic(err instanceof Error ? err.message : "오류가 발생했습니다")
    } finally {
      setIsLoadingTraffic(false)
    }
  }

  const generateReferrerInsights = async () => {
    if (!currentUrlStats) return
    ...
    try {
      ...
      if (!response.ok) throw new Error("AI 분석 생성에 실패했습니다")
      const data = await response.json()
      setReferrerInsights({ referrerAnalysis: data.referrerAnalysis, modelInfo: data.modelInfo })
    } catch (err) {
      setErrorReferrer(err instanceof Error ? err.message : "오류가 발생했습니다")
    } finally {
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button 
                  size="sm" 
                  onClick={generateTrafficInsights} 
            ...
            <CardContent className="space-y-3">
              {trafficInsights?.modelInfo && <ModelInfoBadge modelInfo={trafficInsights.modelInfo} />}
              {errorTraffic && (
                <div className="flex items-center gap-2 text-destructive">
                  <AlertCircle className="h-4 w-4" />
                  <span className="text-sm">{errorTraffic}</span>
                </div>
              )}
              {trafficInsights?.trafficPattern ? (
                <div className="rounded-lg bg-muted/50 p-4">
                  <FormattedInsight text={trafficInsights.trafficPattern} />
                </div>
              ) : (
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Bedrock(Claude) 기반 마케팅 전략 및 타겟 제안
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button 
                  size="sm" 
                  onClick={generateReferrerInsights} 
                  disabled={isLoadingReferrer}
                >
                  {isLoadingReferrer ? (
                    <RefreshCw className="h-4 w-4 animate-spin" />
                  ) : (
            ...
            <CardContent>
              {referrerInsights?.modelInfo && <ModelInfoBadge modelInfo={referrerInsights.modelInfo} />}
              {errorReferrer && (
                <div className="flex items-center gap-2 text-destructive">
                  <AlertCircle className="h-4 w-4" />
                  <span className="text-sm">{errorReferrer}</span>
                </div>
              )}
              {referrerInsights?.referrerAnalysis ? (
                <div className="rounded-lg bg-muted/50 p-4">
                  <FormattedInsight text={referrerInsights.referrerAnalysis} />
                </div>
              ) : (
                <div className="rounded-lg border border-dashed border-border bg-muted/20 p-4">
                  <p className="text-center text-sm text-muted-foreground">
